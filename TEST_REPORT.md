...
最新验证：2026-09-20，本地版本 0.3.0。统一工作台、备课包和回滚验证使用独立测试数据；未调用真实模型、未部署校内正式服务器。下方保留各历史版本记录。
...
- PR #3 合并后，本地现有数据库先经 SQLite Backup API 备份，再启动 0.2.1；完整性、外键和旧记录比对通过，管理密码未改变。3210 健康接口返回 0.2.1，未向原课堂写入测试数据。
